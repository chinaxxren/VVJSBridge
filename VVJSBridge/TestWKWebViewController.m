...
@property(nonatomic, strong) VVJSBridge *bridge;
...
- (void)configureWKWebview {
    ...
    WKUserScript *usrScript = [[WKUserScript alloc] initWithSource:[VVJSBridge handlerJS] injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];
    ...
    [config.userContentController addScriptMessageHandler:self.bridge name:VVJSBridgeName];
...
- (void)getNative:(NSDictionary *)params successCallback:(void (^)(id response))successCallback failureCallback:(void (^)(id response))failureCallback {
    NSLog(@"getNative %@", params);
    if (successCallback) {
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (navigationAction.navigationType == WKNavigationTypeLinkActivated
            && ![hostname containsString:@".waqu.com"]) {
        // 对于跨域，需要手动跳转
...
- (void)dealloc {
    [VVJSBridge cleanHandler:self.bridge];
}
